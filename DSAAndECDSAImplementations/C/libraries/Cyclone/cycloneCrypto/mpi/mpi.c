...
//Dependencies
// #include <Cyclone\cycloneCrypto\core\crypto.h> //"core/crypto.h" //include removed by Cocilova Marco for unused funtionalities
...
void mpiFree(Mpi *r)
{
   ...
   if(r->data != NULL)
   {
      ...
      free(r->data); //modified by Cocilova Marco
      r->data = NULL;
   }
#else
   //Erase contents
...
error_t mpiGrow(Mpi *r, uint_t size)
{
   ...
   if(size > r->size)
   {
      ...
      data = malloc(size * MPI_INT_SIZE); //modified by Cocilova Marco
      ...
      if(data != NULL)
      {
         ...
         if(r->size > 0)
         {
            ...
            memcpy(data, r->data, r->size * MPI_INT_SIZE); //modified by Cocilova Marco

            //Release old memory buffer
            // osMemset(r->data, 0, r->size * MPI_INT_SIZE);
            memset(r->data, 0, r->size * MPI_INT_SIZE); //modified by Cocilova Marco
            // cryptoFreeMem(r->data);
            free(r->data); //modified by Cocilova Marco
         }
         ...
         memset(data + r->size, 0, (size - r->size) * MPI_INT_SIZE); //modified by Cocilova Marco
...
/*__weak_func*/ error_t mpiMul(Mpi *r, const Mpi *a, const Mpi *b) //removed __weak_func tag by Marco Cocilova
{
   error_t error;
   int_t i;
   int_t m;
...
/*__weak_func*/ error_t mpiMulMod(Mpi *r, const Mpi *a, const Mpi *b, const Mpi *p) //removed __weak_func tag by Marco Cocilova
{
   error_t error;
...
/*__weak_func*/ error_t mpiInvMod(Mpi *r, const Mpi *a, const Mpi *p) //removed __weak_func tag by Marco Cocilova
{
   error_t error;
   Mpi b;
   Mpi c;
...
/*__weak_func*/ error_t mpiExpMod(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p) //removed __weak_func tag by Marco Cocilova
{
   error_t error;
   int_t i;
   int_t j;
...
/*__weak_func*/ error_t mpiExpModFast(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p) //removed __weak_func tag by Marco Cocilova
{
...
/*__weak_func*/ error_t mpiExpModRegular(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p) //removed __weak_func tag by Marco Cocilova
{
   //Perform modular exponentiation
   return mpiExpMod(r, a, e, p);
}
 ...
 * @brief Checks if the value is a prime number.
 * @param n The in value to check.
 */
bool_t mpiIsPrime(const Mpi *a)
{
   ...
   //0, 1 and 2 are not prime
   if(a->size == 1 && a->data[0] <= 2)
   {
      return FALSE;
   }

   bool_t flag = TRUE;
   //prepare to cycle until the square root of the prime number to check
   Mpi i;
   Mpi iSquare;
   Mpi modResult;
   mpiInit(&i);
   ...
   mpiInit(&modResult);
   mpiSetValue(&i, 2);
   mpiSetValue(&iSquare, 0);
   //check if we controlled every possible divisor
   while (mpiComp(&iSquare, a) < 0)
   {
      //calculate if 'i' is a divisor of 'a'
      mpiMod(&modResult, a, &i);
      if( modResult.size > 1 && modResult.data > 0)
      {
         //'a' is no prime number
         flag = FALSE;
         break;
      }

      //update flag parameters for next cycle
      mpiMul(&iSquare, &i, &i);
      mpiAddInt(&i, &i, 1);
   }
   mpiFree(&i);
