...
#include <Cyclone\cycloneCrypto\pkc\dsa.h> //"pkc/dsa.h"
#include <Cyclone\cycloneCrypto\mpi\mpi.h> //"mpi/mpi.h"
#include <Cyclone\cycloneCrypto\encoding\asn1.h> //"encoding/asn1.h"
...
/**
 * @brief Non official function: added by Cocilova Marco.
 * @brief Copy a DsaDomainParameter's Mpi values.
 * @param r A pointer to a DsaDomainParameter. (destination)
 * @param params A pointer to a DsaDomainParameter. (source)
 */
error_t dsaDomainParametersCopy(DsaDomainParameters *r, const DsaDomainParameters *params)
{
   if( mpiCopy(&r->g, &params->g) || 
         mpiCopy(&r->p, &params->p) || 
         mpiCopy(&r->q, &params->q) )
   {
...
/**
 * @brief Non official function: added by Cocilova Marco.
 * @brief Checks if the 'p' and 'q' parameters parameters in the struct are right (does not check the 'g' value formula).
 * @param params The DsaDomainParameters to check.
 * @return TRUE is all the parameters respect the DSA variables' rules. FALSE otherwise.
 */
bool_t dsaCheckDomainParameters(const DsaDomainParameters *params)
{
   //check null
   if( (params == NULL) || (&params->g == NULL) || (&params->p == NULL) || (&params->q == NULL) )
   {
      return FALSE;
   }
   ...
   if( mpiComp(&params->g, &t) != 1 || mpiComp(&params->p, &t) != 1 || mpiComp(&params->q, &t) != 1 )
   {
      mpiFree(&t);
      return FALSE;
   }
   ...
   bool_t flag = TRUE;
   // 'q' must be a prime factor of 'p-1', so 'p mod q' must be equal to '1'.
   mpiMod(&t, &params->p, &params->q);
   
   if( !mpiIsPrime(&params->p) || !mpiIsPrime(&params->q) || // 'p' and 'q' must be prime
         mpiComp(&params->p, &params->q) == -1 || mpiCompInt(&t, 1) != 0) // check that ( q <= p-1 ) ^ ( p = 1 (mod q) )
   {
      flag = FALSE;
   }
...
/**
 * @brief Non official function: added by Cocilova Marco.
 * @brief Calculates the g value used for DSA algorithm using the given p and h values.
 * @param g Mpi value generated as result.
 * @param p Mpi prime value used in the DSA algorithm.
 * @param h Mpi secret value only used to generate g.
 */
error_t dsaGenerateGValue(Mpi *g, const DsaDomainParameters *params, const Mpi *h)
{
   if (CRYPTO_STATIC_MEM_SUPPORT == ENABLED)
   {
      return ERROR_NOT_IMPLEMENTED; //I don't know how to manage static memory here...
   }

   error_t error;
   // g = ( h^((p-1)/q) ) (mod p) so that h < p-1 ^ g > 1
   Mpi exp;
   Mpi reminder;
   mpiInit(&exp);
   mpiInit(&reminder);
   ...
   //calculate exp = (p-1)/q
   mpiCopy(&exp, &params->p);
   mpiSubInt(&exp, &exp, 1);
   mpiDiv(&exp, &reminder, &exp, &params->q);

   //calculate g = h^exp (mod p)
   mpiExpMod(g, h, &exp, &params->p);
   ...
   {
      //It shouldn't happen since 'q' is supposed to be lower than 'p-1'
      error = ERROR_FAILURE;
   }
   mpiFree(&exp);
   mpiFree(&reminder);
...
/**
 * @brief Non official function: added by Cocilova Marco.
 * @brief Calculates the respective public key of the given private key.
 * @param pub The DsaPublicKey public key generated as result.
 * @param priv The DsaPrivateKey private key to use to calculate the public key.
 */
error_t dsaGeneratePublicKey(DsaPublicKey *pub, const DsaPrivateKey *priv)
{
   ...
   //clone parameters from priv to pub keys
   mpiCopy(&pub->params.g, &priv->params.g);
   mpiCopy(&pub->params.p, &priv->params.p);
   mpiCopy(&pub->params.q, &priv->params.q);
   //generate pub key value y = g^x (mod p)
   mpiExpMod(&pub->y, &priv->params.g, &priv->x, &priv->params.p);
