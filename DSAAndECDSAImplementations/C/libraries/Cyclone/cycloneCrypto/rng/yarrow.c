...
error_t yarrowInit(YarrowContext *context)
{
   ...
   memset(context, 0, sizeof(YarrowContext)); //function substituted by Cocilova Marco

   //Create a mutex to prevent simultaneous access to the PRNG state
   // if(!osCreateMutex(&context->mutex)) //removed not implemented function by Cocilova Marco (sorry...)
   // {
   ...
   const uint8_t *input, size_t length, size_t entropy)
{
   ...
   // osAcquireMutex(&context->mutex); //removed not implemented function by Cocilova Marco (sorry...)
   ...
   // osReleaseMutex(&context->mutex); //removed not implemented function by Cocilova Marco (sorry...)
...
error_t yarrowRead(YarrowContext *context, uint8_t *output, size_t length)
{
   ...
   while(length > 0)
   {
      ...
      memcpy(output, buffer, n); //function substituted by Cocilova Marco
...
void yarrowDeinit(YarrowContext *context)
{
   ...
   // osDeleteMutex(&context->mutex); //removed not implemented function by Cocilova Marco (sorry...)
