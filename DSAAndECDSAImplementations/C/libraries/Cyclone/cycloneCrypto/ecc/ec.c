 ...
 * @brief Generates a curve point from the function and the x coordinate.
 * @param params The values needed for the function (y^2 = x^3 + ax + b).
 * @param x The x coordinate to calculate the point from.
 * @param r The generated point.
 * @return Error code.
 */
...
/*__weak_func*/ error_t ecAffinify(const EcDomainParameters *params, EcPoint *r, //__weak_func tag removed by Cocilova Marco
   const EcPoint *s)
{
   error_t error;
   Mpi a;
...
/*__weak_func*/ error_t ecMult(const EcDomainParameters *params, EcPoint *r, const Mpi *d,
   const EcPoint *s) //__weak_func tag removed by Cocilova Marco
{
   error_t error;
   uint_t i;
   Mpi h;
