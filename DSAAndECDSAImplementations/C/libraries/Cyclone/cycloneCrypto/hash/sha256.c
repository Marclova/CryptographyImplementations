...
#include <Cyclone\cycloneCrypto\hash\sha256.h> //"hash/sha256.h"
...
/*__weak_func*/ error_t sha256Compute(const void *data, size_t length, uint8_t *digest) //__weak_func tag removed by Cocilova Marco
{
   ...
   context = malloc(sizeof(Sha256Context)); //function substituted by Cocilova Marco
   ...
   free(context); //function substituted by Cocilova Marco
#endif
...
/*__weak_func*/ void sha256Init(Sha256Context *context) //__weak_func tag removed by Cocilova Marco
{
...
/*__weak_func*/ void sha256Update(Sha256Context *context, const void *data, size_t length) //__weak_func tag removed by Cocilova Marco
{
   size_t n;
...
/*__weak_func*/ void sha256Final(Sha256Context *context, uint8_t *digest) //__weak_func tag removed by Cocilova Marco
{
   uint_t i;
   size_t paddingSize;
   uint64_t totalSize;
...
/*__weak_func*/ void sha256FinalRaw(Sha256Context *context, uint8_t *digest) //__weak_func tag removed by Cocilova Marco
{
   uint_t i;
